...
import { ref, uploadBytes, getDownloadURL } from 'firebase/storage';
  ...
  const ScienceAdminMeritList = () => {
    ...
    useEffect(() => {
      const unsubscribe = onSnapshot(collection(db, 'ScienceClgAcceptedApplications'), (snapshot) => {
    ...
    const uploadPDF = async (pdfUri) => {
      ...
      const storageRef = ref(storage, `ScienceClgMeritLists/${pdfName}.pdf`);
      const metadata = {
        customMetadata: {
          timestamp: timestamp
        }
    ...
    const deleteDocuments = async () => {
      ...
      filteredStudents.forEach(student => {
        const docRef = doc(firestore, 'ScienceClgAcceptedApplications', student.id);
        batch.delete(docRef);
      });
      await batch.commit();
    };
...
export default ScienceAdminMeritList;
