...
const ScienceClgAdminFrame = () => {
  const [applications, setApplications] = useState([]);
  const [selectedApplication, setSelectedApplication] = useState(null);
  const [loading, setLoading] = useState(true);
  
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'ScienceClgStudentsInfo'), (snapshot) => {
      ...
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            await updateDoc(doc(db, 'ScienceClgStudentsInfo', application.id), {
              status: 'accepted',
            });
  
            // Add the accepted application to Firestore
            await addDoc(collection(db, 'ScienceClgAcceptedApplications'), {
              ...application,
              status: 'accepted',
            });
...
export default ScienceClgAdminFrame;
